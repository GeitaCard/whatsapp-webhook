// ============================================================
// GEITACARD - INDEX.JS
// MULTI EVENT + WHATSAPP + SUPABASE + EXCEL/CSV + QR + CHECK-IN
// ============================================================
...
function normalizePhone(phone) {
  let value = clean(phone).replace(/[^\d+]/g, "");
...
app.post("/api/events", async (req, res) => {
  try {
    const eventKey =
      upper(req.body.event_key);

    const eventName =
      clean(req.body.event_name) ||
      eventKey;

    const templateName =
      clean(req.body.template_name);

    const templateLanguage =
      clean(req.body.template_language) ||
      "sw";

    const cardBase64 =
      clean(req.body.card_image_base64);

    if (!eventKey) {
      return res.status(400).json({
        success: false,
        error: "Event Key inahitajika."
      });
    }
  ...
  try {
    ...
    if (!event) {
      return res.status(404).json({
        success: false,
        error:
          `Event ${selectedEventKey} haipo au imezimwa.`
      });
    }
...
//
// Kama Event column ipo,
// kila row inatumia Event yake.
//
// Kama Event column haipo,
// row inatumia Event uliyochagua Dashboard.
// ============================================================
    ...
    try {
      ...
      if (!selectedEventKey) {
        return res.status(400).json({
          success: false,
          error:
            "Event ya msingi haijachaguliwa."
        });
      }
      ...
      for (
        ...
        try {
          ...
          if (!rowEvent) {
            throw new Error(
              `Event ${rowEventKey} haipo au imezimwa.`
            );
          }

          // --------------------------------------------------
          // EVENT MUST HAVE CARD
          // --------------------------------------------------

          if (!rowEvent.card_image_url) {
            throw new Error(
              `Event ${rowEventKey} haina Card URL.`
            );
          }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Hapa ndipo replies za WhatsApp
      // zinaweza kushughulikiwa baadaye.
